...
9. In the target we can select the ARM compiler, as well as the Freq(cLK) we want to run our system.(Selecting clocks is not available in Keil uvision 5 and higher), we can     also "simulate" the code if we want, but that has its own limitations.
