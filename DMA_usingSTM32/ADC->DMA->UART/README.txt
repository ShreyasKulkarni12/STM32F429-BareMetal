...
There are problems with this code ATM wherein, only the 1st instance of the ADC value is stored and given to the DMA[i.e, the ADC
is not running in continous mode because the feature is not available], which the DMA is transmitting. Though using ADC->DMA and then to USART is unneccessary, the sole purpose of the code is to understand and interface 2 peripherals using 2 DMA streams simaltenously.
